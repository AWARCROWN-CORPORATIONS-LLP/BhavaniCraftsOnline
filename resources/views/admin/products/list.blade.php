...
@section('header_extra')
    <div class="flex items-center space-x-4">
        <h2 class="text-xl lg:text-3xl font-black text-gray-900 uppercase tracking-tighter">Master Catalog</h2>
        <a href="{{ route('admin.products.create') }}" class="btn-luxury-saffron px-6 py-2 text-[10px] shadow-xl">Register New Master Artifact</a>
    </div>
@endsection
            ...
            <table class="w-full text-left">
                <thead class="bg-gray-50/50">
                    <tr>
                        ...
                        <th class="p-8 text-[10px] font-black text-gray-400 uppercase tracking-[3px]">Market Value (MRP)</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($products as $product)
                        <tr class="hover:bg-gray-50/50 transition-all group">
                            <td class="p-8">
                                <div class="flex items-center space-x-4">
                                    <div class="h-16 w-16 rounded-xl bg-gray-100 flex items-center justify-center font-black text-gray-300 group-hover:bg-[#ff9933]/10 group-hover:text-[#ff9933] transition-all overflow-hidden border border-gray-100 uppercase shrink-0">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex items-center space-x-2">
                                            <p class="text-[10px] text-[#ff9933] font-bold uppercase tracking-wider leading-none">{{ $product->category->name ?? 'Unmapped' }}</p>
                                            <span class="text-[8px] px-2 py-0.5 rounded bg-gray-100 text-gray-500 font-bold uppercase tracking-widest">
                                                {{ $product->user ? 'Partner: ' . $product->user->name : 'Master Forge' }}
                                            </span>
                                        </div>
                            ...
                            <td class="p-8">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <span class="text-[9px] text-gray-300 font-bold uppercase tracking-widest">Units in Registry</span>
                                </div>
                            </td>
                            ...
                            <td class="p-8 text-right">
                                <div class="flex items-center justify-end space-x-4">
                                    ...
                                    <form action="{{ route('admin.products.destroy', $product->id) }}" method="POST" class="inline" onsubmit="return confirm('Eradicate entity from catalog?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="p-20 text-center text-gray-400 lowercase italic opacity-20">registry empty. begin foundation.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
