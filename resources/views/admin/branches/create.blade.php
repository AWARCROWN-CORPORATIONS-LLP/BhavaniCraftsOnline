...
@section('header_extra')
    <div class="flex items-center space-x-4">
        <h2 class="text-xl lg:text-3xl font-black text-gray-900 uppercase tracking-tighter">New Branch</h2>
        <span class="text-gray-300">/</span>
        <p class="text-[10px] font-black text-[#ff9933] uppercase tracking-[4px]">Physical Location Registry</p>
    </div>
@endsection
...
@section('content')
    ...
    <div class="max-w-4xl mx-auto">
        <form action="{{ route('admin.branches.store') }}" method="POST" class="card-premium p-10 space-y-10">
            @csrf
                ...
                <div class="space-y-8">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" required value="{{ old('name') }}" placeholder="e.g. Hyderabad Flagship"
                        ...
                        <div>
                            ...
                            <input type="text" name="city" value="{{ old('city') }}" placeholder="e.g. Hyderabad"
                                   class="w-full bg-gray-50 border-none px-6 py-4 rounded-2xl text-sm font-medium focus:ring-2 focus:ring-[#ff9933]/30">
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="email" name="email" value="{{ old('email') }}" placeholder="branch@bhavanicrafts.com"
                                       class="w-full bg-gray-50 border-none px-6 py-4 rounded-2xl text-sm font-medium focus:ring-2 focus:ring-[#ff9933]/30">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <textarea name="address" rows="3" placeholder="Street, Building, Area, Pin Code..."
                                      class="w-full bg-gray-50 border-none px-6 py-4 rounded-2xl text-sm font-medium focus:ring-2 focus:ring-[#ff9933]/30">{{ old('address') }}</textarea>
                        </div>

                        <div>
                            <label class="text-[10px] font-black text-gray-600 tracking-[3px] uppercase mb-3 block leading-none">Google Maps Link</label>
                            <input type="url" name="map_link" value="{{ old('map_link') }}" placeholder="https://goo.gl/maps/..."
                                   class="w-full bg-gray-50 border-none px-6 py-4 rounded-2xl text-sm font-medium focus:ring-2 focus:ring-[#ff9933]/30">
                        </div>
            ...
            <div class="pt-8 flex items-center justify-between border-t border-gray-50">
                <a href="{{ route('admin.branches.index') }}" class="text-[10px] font-black text-gray-400 uppercase tracking-widest hover:text-red-500 transition-colors">← Cancel Registration</a>
                <button type="submit" class="btn-luxury-saffron px-12 py-5 text-[11px] shadow-lg">Authenticate & Create Branch</button>
            </div>
        </form>
    </div>
