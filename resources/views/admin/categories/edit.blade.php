...
@section('content')
    ...
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="card-premium p-10 space-y-10">
            @csrf @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div x-data="{ preview: '{{ $category->display_image }}', hasExisting: {{ $category->image_path ? 'true' : 'false' }} }" class="space-y-4">
                    ...
                    <label for="image-upload-edit"
                        ...
                        <div x-show="!preview" class="flex flex-col items-center space-y-3 text-gray-400">
                            ...
                            <p class="text-sm font-bold text-gray-600">Click to upload image</p>
                            <p class="text-xs text-gray-400">PNG, JPG, GIF, WebP — Max 2MB</p>
                        </div>

                        <input id="image-upload-edit" type="file" name="image" accept="image/*" class="sr-only"
            ...
            <div class="pt-6 flex items-center justify-between border-t border-gray-50">
                ...
                <button type="submit" class="btn-luxury-saffron px-10 py-4 text-[11px] shadow-lg">Save Changes</button>
            </div>
        </form>
    </div>
