...
@section('header_extra')
    <div class="flex items-center space-x-4">
        <h2 class="text-xl lg:text-3xl font-black text-gray-900 uppercase tracking-tighter">Franchise Approvals</h2>
        <span class="bg-[#1e40af]/10 text-[#1e40af] text-[9px] font-black uppercase tracking-[3px] px-4 py-1.5 rounded-full border border-[#1e40af]/20">Approval Needed</span>
    </div>
@endsection
...
@section('content')
    ...
    <div class="card-premium overflow-hidden">
        <div class="p-8 border-b border-gray-100 flex items-center justify-between">
            <h3 class="text-xs font-black text-gray-400 uppercase tracking-[6px] leading-none">Pending Partners</h3>
            ...
            <table class="w-full text-left">
                <thead class="bg-gray-50/50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-[3px]">Request Date</th>
                        <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-[3px]">Permissions</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($businessAccounts as $user)
                        <tr class="hover:bg-gray-50/50 transition-all duration-300 group">
                            ...
                            <td class="px-6 py-4">
                                @if($user->is_approved)
                                    <span class="px-5 py-2 bg-green-50 text-green-600 text-[9px] font-black uppercase tracking-[3px] rounded-full border border-green-100 shadow-sm shrink-0">Approved</span>
                                @else
                                    <span class="px-5 py-2 bg-red-50 text-red-600 text-[9px] font-black uppercase tracking-[3px] rounded-full border border-red-100 shadow-sm animate-pulse shrink-0">Pending Approval</span>
                                @endif
                            </td>
                                ...
                                @if(!Auth::user()->hasRole('employee'))
                                    @if(!$user->is_approved)
                                        <form action="{{ route('admin.approve_franchise', $user->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button class="btn-luxury-saffron px-8 py-3 text-[10px] shadow-2xl">Approve Partner</button>
                                        </form>
                                    @else
                                        <button disabled class="px-8 py-3 bg-gray-100 text-gray-400 rounded-xl font-black uppercase tracking-[2px] text-[10px] cursor-not-allowed">Status Approved</button>
                                    @endif
                                @else
                                    <span class="text-[9px] text-gray-400 font-bold uppercase tracking-widest">Access Denied</span>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="py-12 text-center">
                                <div class="flex flex-col items-center opacity-30">
                                    ...
                                    <h4 class="text-xl font-black uppercase tracking-[10px] text-gray-400">No Pending Requests</h4>
