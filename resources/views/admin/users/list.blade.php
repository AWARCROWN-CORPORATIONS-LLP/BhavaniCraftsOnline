...
@section('header_extra')
    <div class="flex items-center space-x-4">
        <h2 class="text-xl lg:text-3xl font-black text-gray-900 uppercase tracking-tighter">Seeker Registry</h2>
        <span class="bg-gray-100 text-gray-400 text-[9px] font-black uppercase tracking-[3px] px-4 py-1.5 rounded-full border border-gray-200">Global Users</span>
    </div>
@endsection
...
@section('content')
    ...
    <div class="card-premium overflow-hidden">
        <div class="p-8 border-b border-gray-100 flex items-center justify-between">
            <h3 class="text-xs font-black text-gray-400 uppercase tracking-[6px] leading-none">Registered Members</h3>
            <span class="text-[9px] font-black uppercase text-gray-300">Total: {{ count($users) }} Identities</span>
        </div>
            ...
            <table class="w-full text-left">
                <thead class="bg-gray-50/50">
                    <tr>
                        ...
                        <th class="p-8 text-[10px] font-black text-gray-400 uppercase tracking-[3px]">Role & Access</th>
                        <th class="p-8 text-[10px] font-black text-gray-400 uppercase tracking-[3px]">Contact Info</th>
                        <th class="p-8 text-[10px] font-black text-gray-400 uppercase tracking-[3px]">Registry Date</th>
                        ...
                        <tr class="hover:bg-gray-50/50 transition-all group">
                            ...
                            <td class="p-8">
                                ...
                                @elseif($user->hasRole('admin'))
                                    <span class="px-5 py-2 bg-orange-50 text-orange-600 text-[9px] font-black uppercase tracking-[3px] rounded-full border border-orange-100">Master Admin</span>
                                @elseif($user->hasRole('franchise'))
                                    <span class="px-5 py-2 bg-blue-50 text-blue-600 text-[9px] font-black uppercase tracking-[3px] rounded-full border border-blue-100">Partner Hub</span>
                                @else
                                    <span class="px-5 py-2 bg-gray-50 text-gray-400 text-[9px] font-black uppercase tracking-[3px] rounded-full border border-gray-200">Customer Seeker</span>
                                @endif
                            </td>
                            <td class="p-8">
                                <div class="space-y-1">
                            ...
                            <td class="p-8 text-right">
                                @if(Auth::id() !== $user->id)
                                    @if(Auth::user()->hasRole('employee') && ($user->hasRole('super_admin') || $user->hasRole('admin') || $user->hasRole('franchise')))
                                        <span class="text-[9px] text-gray-300 font-black uppercase tracking-widest">Restricted Entity</span>
                                    @else
                                        <form action="{{ route('admin.toggle_block', $user->id) }}" method="POST" class="inline">
                                            ...
                                            <button type="submit" 
                                                ...
                                                {{ $user->is_blocked ? 'Unblock Identity' : 'Suspend Access' }}
                                            </button>
                                        </form>
                                    @endif
                                @else
                                    <span class="text-[9px] text-gray-300 font-black uppercase tracking-widest">Master Root</span>
                                @endif
                            </td>
                        </tr>
                    @empty
